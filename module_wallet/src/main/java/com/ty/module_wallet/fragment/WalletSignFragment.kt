...
class WalletSignFragment : BaseFragment(), ConnectCallback {
    ...
    fun showConnectBottomDialog(
        ...
        btn_connect.setOnClickListener {
            DAppConnect.sendConnectResponse(id, address, true, walletInfo, false, null)
            bottomSheetDialog!!.dismiss()
            connectCallback?.connectApprove()
        }
        btn_cancel.setOnClickListener {
            DAppConnect.sendConnectResponse(id, address, false, null, false, null)
            bottomSheetDialog!!.dismiss()
                ...
                DAppConnect.sendSignResponse(
                    ...
                    object : SendBridgeMessageCallback {
                        override fun onReceived() {
                            Log.i("SendBridgeMessageCallback","onReceived")
//                            Toast.makeText(requireActivity(), "onReceived", Toast.LENGTH_SHORT).show()
                        }

                        override fun onFail() {
                            Log.i("SendBridgeMessageCallback","onFail")
//                            Toast.makeText(requireActivity(), "onFail", Toast.LENGTH_SHORT).show()
                        }

                        override fun onTimeout() {
                            Log.i("SendBridgeMessageCallback","onTimeout")
//                            Toast.makeText(requireActivity(), "onTimeout", Toast.LENGTH_SHORT).show()
                        }
                    })
                bottomSheetDialog!!.dismiss()
        ...
        btn_cancel.setOnClickListener {
            if (onSignCallback != null) {
                DAppConnect.sendSignResponse(id, false, null, false, null)
                bottomSheetDialog!!.dismiss()
    ...
    override fun onSuccess() {
        DAppConnect.init(dAppID!!, object : BridgeConnectCallback {
        ...
        DAppConnect.setTargetTopicID(topicId!!)
        DAppConnect.setTargetPubKey(pubKey!!)
    }
